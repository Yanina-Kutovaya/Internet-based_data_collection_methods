...
def page_data(df, site_id, position, page_n, headers):
    ...
    k = site_dict[site_id]['k']
    empty_clause = site_dict[site_id]['empty_clause']
    s_str = site_dict[site_id]['s_str']

    n = len(df)
    for i in range(len(div)):
        a = div[i].findChildren()[child_n].get('href')
        pattern = re.compile('[\d]*')
        df[n + i] = {'id': pattern.findall(a)[k],
                     'name': div[i].findChildren()[child_n].text.replace('\xa0', ' '),
                     'link': site_dict[site_id]['link_prefix'] + a,
