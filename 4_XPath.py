...
def to_date(a):
    s = datetime.datetime.today().date().strftime('%d.%m.%Y')
    if re.findall(':', a) == [':']:
        a = s
    else:
        s1 = a.split(' ')[0]
...
def mail_ru(headers, df):
    ...
    df[n] = {'source': root.xpath(s + "//span[@class='newsitem__param']/text()")[1],
             ...
             'date': to_date(root.xpath(s + "//span[@datetime]/text()")[1])}
    ...
    for i in range(len(items)):
        ...
        date = to_date(html.fromstring(r).xpath(s + "//span[@datetime]/text()")[0])
        df[n + i + 1]['date'] = date
